...
Sub Fmt_AutoColorSelection_A()
    ...
    '    Set cell = Nothing
    '    Set constantCell = Nothing
    '    Set formulaCells = Nothing
    '    Set cellFormula = Nothing
    '
End Sub
...
Function Fnc_GetText(CellRef As String)
    ...
    Dim StringLength As Integer, i As Integer, Result As Double
    StringLength = Len(CellRef)
    For i = 1 To StringLength
...
Sub Tool_Load2RecentColors()
    ...
    Dim CurrentFill As Variant, x As Variant, currentColor As Variant
...
Sub Tool_TurnAutoFitOff_PivotTables()
    'PURPOSE: Turn off Autofit Column Width On Update Setting for every Pivot Table
    'SOURCE: www.TheSpreadsheetGuru.com/the-code-vault

    Dim pvt    As PivotTable
    Dim OnOff  As Integer
    OnOff = Application.InputBox("Select an option" & _
            "   (Must be 1-2)" & vbCr & vbCr & _
            "   [1] Off - Turn off autofit for all pivot tables " & vbCr & _
            "   [2] On - Turn on autofit for all pivot tables " & vbCr & _
            " ", Type:=1, Title:="Align Shapes")
    
    If OnOff <> 1 And OnOff <> 2 Then
        MsgBox "You didnt enter a valid option"
        Exit Sub
    ElseIf OnOff = 1 Then
...
Sub Tool_WrapIfError_v2()
    ...
    Dim x      As String
    Dim Beg_String As Variant, End_String As Variant
